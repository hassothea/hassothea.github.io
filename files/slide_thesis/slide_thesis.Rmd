...
class: left, top
### Notation<hbr>
- $(X,Y)\in{\cal X}\times{\cal Y}$: input-output generic data and space.
    
  - ${\cal X}=\mathbb{R}^d$
    
  - ${\cal Y}$ is either $\mathbb{R}$ (regression) or $\{1,...,N\}$ ( $N$-class classification).

- ${\cal D}_n=\{(X_1,Y_1),...,(X_n,Y_n)\}$: $iid$ training data of size $n$.
...
$$\underbrace{\{{\cal M}_{1,1},{\cal M}_{1,2},{\cal M}_{1,3}\}}_{{\cal M}_1}$$
...
$$\underbrace{\{{\cal M}_{2,1},{\cal M}_{2,2},{\cal M}_{2,3}\}}_{{\cal M}_2}$$

<br>
<h1br>

.center[.stress[Combining]] 
$$({\cal M}_1, {\cal M}_2, {\cal M}_3)$$
]
...
$$\underbrace{\{{\cal M}_{3,1},{\cal M}_{3,2},{\cal M}_{3,3}\}}_{{\cal M}_3}$$ 
...
exclude: true
## Candidate models<hbr> .subtitle[&nbsp; Step F]<hbr>
- .stress[Step K] : .stress[K-means] with $M$ options of Bregman divergences ${\cal B}_m, m=1,...,M$, are implemented. We then have $M$ partition structures of in put data:
$${\cal C}_1,...,{\cal C}_M$$
where ${\cal C}_m=\{C_{m,1},...,C_{m,K}\}$ is the partition corresponding to ${\cal B}_m$.
...
- .stress[Step F] : fit simple local model on each obtained cluster.
  ...
  We then have $M$ candidate models: $${\cal M}_1, ..., {\cal M}_M$$
  where ${\cal M}_m=\{{\cal M}_{m,1},...,{\cal M}_{m,K}\}$ with ${\cal M}_{m,k}$ built on $C_{m,k}$.

---
## Combining methods<hbr> .subtitle[&nbsp; Step C]<hbr>
### Classification
- Example: binary classification
  - With $4$ classifiers $({\cal M}_1,{\cal M}_2,{\cal M}_3,{\cal M}_4)$
  - New observation $x$ with predicted classes : $({\cal M}_1,{\cal M}_2,{\cal M}_3,{\cal M}_4)=$ (.red[1,1,0,1]).

| $i$ | ${\cal M}_1$| ${\cal M}_2$ | ${\cal M}_3$ | ${\cal M}_4$ | $Y$ |
|------|-------------|--------------|-----------------------------|-----
| $1$ | 0 | 1 | 0 | 1 | 0 |
...
- ${\cal D}_n={\cal D}_k\cup{\cal D}_{\ell}$.

- .stress[Step K] and .stress[F] are based only on ${\cal D}_k$, and .stress[Step C] is done based on ${\cal D}_{\ell}$.
...
- Quadratic risk : ${\cal R}(f)=\mathbb{E}(|f(X)-g^*(X)|^2)$
...
$$\mathbb{E}[|g_n({\bf r}_k(X))-g^*(X)|^2]\leq \inf_{f\in{\cal G}}\mathbb{E}[|f({\bf r}_k(X))-g^*(X)|^2]\\
...
$$\Big[\exists m_0:{\cal R}(r_{k,m_0})\xrightarrow[k\to+\infty]{} 0\Big]\Rightarrow \Big[{\cal R}(g_n({\bf r}_k(.)))\xrightarrow[k,\ell\to+\infty]{}0\Big]$$
...
- Uncorrelated case : $X\sim{\cal U}[-1,1]^d$, average RMSEs over $100$ runs.
<img src="./img/uncor.png" height="200px" width="700px"/>

- Uncorrelated case : $X\sim{\cal N}_d(0, \Sigma)$ with $\Sigma_{ij}=2^{-|i-j|}$.
...
- Prediction matrix : ${\bf r}_k({\cal X})\in\mathbb{R}^{\ell\times M}$.
...
$$\tilde{\bf r}_k({\cal X})={\bf r}_k({\cal X})\times G\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\ \\
=\begin{pmatrix}
